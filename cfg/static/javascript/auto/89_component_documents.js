var Component_Documents = Class.create(Lightbox, {
	...
	initialize: function(prefix) {
		...
		this.panels.select ('div.ep_upload_doc').each(function (doc_div) {
			...
			//RM override the roll up if more than one rule too
			if($(prefix + '_panels').select('div.ep_upload_doc').length > 1){
				jQuery(doc_div).find(".ep_upload_doc_expansion_bar > a").click();
			}
		});
...
//			var opt_a = jQuery(doc_div).find(".ep_upload_doc_expansion_bar > a");
//			console.log(jQuery(doc_div).find(".ep_upload_doc_expansion_bar > a").html());
//			jQuery(opt_a).trigger("click");
