...
package EPrints::Plugin::InputForm::Surround::Default_lshtm_large;
...
sub render
{
	...
	# style for title bar
	my $title_bar = $self->{session}->make_element( "div", class=>"ep_sr_title_bar_lshtm_large $title_bar_class", id=>$barid );
	my $title_div = $self->{session}->make_element( "div", class=>"ep_sr_title_lshtm" );

	my $content = $self->{session}->make_element( "div", id => $component->{prefix}."_content", class=>"$content_class ep_sr_content_lshtm_large" );
	...
	if( $component->is_collapsed )
	{
		...
		my $col_div = $self->{session}->make_element( "div", class=>"ep_sr_collapse_bar_lshtm_large ep_only_js ep_toggle", id => $colbarid );
