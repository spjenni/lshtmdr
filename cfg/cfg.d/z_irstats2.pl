...
$c->{plugins}->{"Screen::IRStats2::Report"}->{appears}->{key_tools} = 1;
...
#$c->{plugins}->{"Screen::EPrint::Box::Stats"}->{appears}->{summary_bottom} = 43;
#$c->{plugins}->{"Screen::EPrint::Box::Stats"}->{appears}->{summary_right} = undef;