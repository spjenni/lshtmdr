...
#	{ id=>"types", allow_null=>0, fields=>"type" },
	{ id=>"divisions", allow_null=>0, fields=>"divisions" },
    { id=>"funders", allow_null=>0, fields=>"funders" },
    { id=>"collection_mode", allow_null=>0, fields=>"collection_mode" },
];
...
$oai->{custom_sets} = [
    ...
    { spec => "openaire_data", name => "Openaire Data", filters => [
		{ meta_fields => [ "funders" ], match=>"SET" },
	] },
    { spec => "datacite", name => "DataCite compliant items", filters => [
		{ meta_fields => [ "id_number" ], match=>"SET" },
	] },
